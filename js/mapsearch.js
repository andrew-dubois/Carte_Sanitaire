...
function viewSearchResult() {
    ...
    $.get("lib/inc/orgunitDHIS.inc.php?qfacname5=" + valField, function (data) {
        ...
        $.each(obj, function () {
            namefac = this['name'];
            id = this['id'];
            depfac = this['department'];
            commfac = this['commune'];
...
function viewSearchResult_occ() {
    ...
    $.get("lib/inc/orgunitDHIS.inc.php?qfacname=" + valField, function (data) {
        var obj = jQuery.parseJSON(data);
        var result = '<table class="table table-condensed table-hover table-striped">';
        //if no result
        if (jQuery.isEmptyObject(obj)) {
