...
function initialize(){
	...
	var styledMap = new google.maps.StyledMapType(styles,	{name: "Principale"});
		...
		//Associate the styled map with the MapTypeId and set it to display.
		  ...
		  //runColorizeHTI();//map delimitation
		  
		  /*//test chart on mapseries_val_color
		  series_val_color=new Array();
		  series_val_color.push([12,'175F9C']);
		  series_val_color.push([33,'33a2f7']);
		  series_val_color.push([55,'8fc0e8']);
		  //alert(series_val_color);
		  
		  
		  iconImage=iconimagechart(128,series_val_color);
		  ...
		  position: new google.maps.LatLng(19.308551284387825,-72.47448921203613),
		  icon:iconImage,
		  map: map,
		  title:'test chart'
		  });*/
		...
		//view all facilities by communes with boundaries
		//viewAllFac_by_comm('Haiti');
...
/*****************************************GOOGLE MAP EVENTS****************************************/
		...
		//use to get latlng on the map -its a test fnction 
		 google.maps.event.addListener(map, 'click', function(ev) {
         document.getElementById('test').innerHTML="Lat:"+ev.latLng.lat()+" Longi:"+ ev.latLng.lng();
         });
		
/*****************************************END GOOGLE MAP EVENT*************************************/		

/*****************************************COMPONENTS EVENT*****************************************/
		//Department Boundaries management
		$('#datap-dep-list').on('change',function(){					
				//var nameDep=$('#datap-dep-list').val();
				//viewAllFac_by_comm(nameDep);
